...
class Rtcx:
    ...
    def freeze(self, iotId, orderId):
        params = {
            "iotId": iotId,
            "orderId": orderId,
            "status": 2
        }
        path = "/platform/vision/customer/cloudstorage/status/set"
        baseData["params"] = params
        result = rtcxUtil.request(path, baseData, self.rtcx_url)
        print(f"冻结：{iotId}，结果是：{result}")

    def getOrderList(self,iotId):
        params = {
            "iotId": iotId
        }
        ...
        if data["code"] == 200:
            if data["data"] and data["data"]["orderList"]:
                for order in data["data"]["orderList"]:
                    if order["status"] == 0:
                        self.freeze(iotId, order["orderId"])



    def sendMsg(self, openId, data):
        params = {
            "openId": openId,
            "message": json.dumps(data),
        }
        path = "/platform/message/user/transfer"
        baseData["params"] = params
        result = rtcxUtil.request(path, baseData, self.rtcx_url)
        print(f"发送消息：{data}，结果是：{result}")

    # 相速购买接口
    def buy(self, iotId, openId, model):
        product = productsXX[self.env][model]
    ...
    def products(self):
        params = {}
        path = "/platform/customer/cloudstorage/commodity/query"
        baseData["params"] = params
        result = rtcxUtil.request(path, baseData, self.rtcx_url)
        data = json.loads(result)
        print(json.dumps(data, sort_keys=True, indent=4, ensure_ascii=False))
        ...
        if data and len(data) > 0:
            ...
            if rows and len(rows) > 0:
                for row in rows:
                    ...
                    self.buy(iotId, openId, productModel)
        else:
            print("用户不存在")
        ...
        if meta:
            ...
            if bindInfo:
                for user in bindInfo:
                    ...
                    print(
                        f"绑定信息：用户Id:{userId} openId:{user['open_id']} iotId:{user['iot_id']}  绑定状态是：{bind_status} 绑定类型是：{bindType}")
                    database = "ugreen_dpt_user_" + self.env
...
if __name__ == '__main__':
    ...
    account = "ningna@ugreen.com"

    sn = "I50000U57Q200013"

    # 查询账号下的，所有绑定情况
    # rtcx.bindInfo(account)

    # 查询账号下全部历史绑定状态
    # rtcx.allBindInfo(account)

    # 根据手机号，查询账号下面的设备，全部开通云存储
    # rtcx.buyBuyPhone(account)
    # rtcx.buyBuySN(sn)

    # 获取某个设备绑定关系
    # rtcx.getSnInfo(sn)

    # rtcx.getIotBuy("DLddgSfDzOiKkHAENzNJGc82ck9KIRXfVuXx94YK")

    # moble = "eadcf5519885333cb6642e945857a50a"
    # phonex = AESUtil.decrypt(key, moble)
    # print(phonex)
    ...
    openId = "AjdFpc-f34hq3oG5V1cT-O35f_Xo"
    # rtcx.sendMsg(openId, msg)

    # zDqBCDBfxCVGNKjwQkasTOAYbTj9y1duF1EGTLre, zDqBCDBfxCVGNKjwQkasT3ZzFPgHfMosIfLNrFG1
    iotId = "zDqBCDBfxCVGNKjwQkas0UJMmXtYdSPXUX334Pfz"
    rtcx.getOrderList(iotId)
