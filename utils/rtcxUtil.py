...
def handler(path, body_dict,url):
    # 构造 headers
    ...
    xx = url + path
    response = requests.post(xx, headers=headers, data=body_str)
...
def getToken(url):
    ...
    result = handler(path, data,url)
    json_result = json.loads(result)
    return json_result["data"]["accessToken"]


def request(path, body_dict,url):
    token = getToken(url)
    body_dict["request"]["cloudToken"] = token
    return handler(path,body_dict,url)


if __name__ == '__main__':
    url = 'https://api-test.aminer.cn'
    getToken(url)
